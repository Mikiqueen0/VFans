    ...
    return (
        <nav className="sticky top-0 bg-primary grid grid-cols-[30%_40%_30%] max-sm:grid-cols-[20%_60%_20%] justify-between items-center h-[5rem] z-50">
            <div className="ml-[5rem] max-xl:ml-[1.5rem] flex items-center gap-10 max-sm:gap-3">
                <button className="xl:hidden flex flex-col justify-center items-center group z-50" ref={hamburgerButtonRef} onClick={() => setHamburger(!hamburger)}>
                    <div className={`${genericHamburgerLine} ${hamburger ? "rotate-45 translate-y-[0.64rem] group-hover:opacity-100" : "group-hover:opacity-100"}`} />
                    <div className={`${genericHamburgerLine} ${hamburger ? "opacity-0" : "group-hover:opacity-100"}`} />
                    <div className={`${genericHamburgerLine} ${hamburger ? "-rotate-45 -translate-y-[0.64rem] group-hover:opacity-100" : "group-hover:opacity-100"}`} />
                </button>
                <a href="/home">
            ...
            <div className="bg-white h-[2.8rem] rounded-[20px] flex items-center p-2">
                <img src={searchIcon} alt="search" className="opacity-90 h-[1.3rem] ml-2" />
                {/* {path !== "" && 
                ...
                <input type="text" placeholder="Search" className="font-normal text-[14px] text-black text-opacity-[78%] bg-white ml-[0.4rem] mr-[1rem] h-full flex-1 focus:outline-none min-w-[6rem]" />
            </div>
                ...
                <div className="flex flex-row items-center justify-end">
                    ...
                    <Menu as="div" className="relative inline-block text-left">
                        ...
                        <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                            ...
                            <div className="py-1">
                                <Menu.Item>
                                {({ active }) => <a href="#" className={classNames(active ? 'bg-dark-background text-opacity-90' : 'text-opacity-[78%]', 'block px-4 py-2 text-sm text-white')}>
                                    <img src={profileIcon} alt="profile" className="inline-flex h-5 mr-2" />
                                    Profile
                                    </a>}
                                </Menu.Item>
                                <Menu.Item>
                                {({ active }) => <a href="#" className={classNames(active ? 'bg-dark-background text-opacity-90' : 'text-opacity-[78%]', 'block px-4 py-2 text-sm text-white')}>
                                        <img src={settingIcon} alt="setting" className="inline-flex h-5 mr-2" />
                                    Setting
                                    </a>}
                                </Menu.Item>
                                <Menu.Item>
                                {({ active }) => <a href="/login" className={classNames(active ? 'bg-dark-background text-opacity-90' : 'text-opacity-[78%]', 'block px-4 py-2 text-sm text-white')}>
                                        <img src="" alt="login" className="inline-flex h-5 mr-2" />
                                    Login
                                    </a>}
                                </Menu.Item>
                                <Menu.Item>
                                {({ active }) => <a href="/signup" className={classNames(active ? 'bg-dark-background text-opacity-90' : 'text-opacity-[78%]', 'block px-4 py-2 text-sm text-white')}>
                                ...
                                <form method="POST" action="#">
                                    <Menu.Item>
                                        {({ active }) => <button type="submit" className={classNames(active ? 'bg-dark-background text-white text-opacity-90' : 'text-opacity-[78%]', 'block w-full px-4 py-2 text-left text-sm text-white')}>
                                            <img src={logoutIcon} alt="logout" className="inline-flex w-5 mr-2" />
                                            Sign out
                                        </button>}
                                    </Menu.Item>
                                </form>
