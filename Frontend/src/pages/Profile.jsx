...
//           console.log(fetchProfile.data.user, "fetchProfile.data.user");
//         } else {
...
//       .from("VFans")
//       .upload(profileDataCopy._id + "/" + uuidv4(), file);
//     if (data) {
//       getMedia();
//     } else {
//       console.log(error);
  ...
  const handleBannerChange = async (e) => {
    ...
    try {
      const { data } = await supabase.storage
        .from("VFans")
        .upload("/" + uuidv4(), file);
      const banner = await supabase.storage
        .from("VFans")
        .getPublicUrl(data.path);
