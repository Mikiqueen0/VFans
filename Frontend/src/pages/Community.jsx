...
export default function Community() {
  ...
  useEffect(() => {
    const fetchCommunity = async () => {
      setLoading(true);
      try {
        const { data: fetchCommunityData } = await axios.get(
  ...
  return (
    <div className="bg-dark-background scrollbar-thin">
      ...
      <div className="flex flex-row justify-center min-h-[100vh] pt-[1.25rem] pb-[1.25rem] z-40">
        ...
        {!loading && (
          <section className="flex flex-col gap-3 max-sm:px-[1rem] px-[1rem] w-[800px] text-white">
            {/* community info */}
            <div
              className={`w-full h-[20rem] min-h-[20rem] rounded-[10px] flex flex-col justify-end relative`}
            >
              <img
              ...
              <div className="relative w-full min-h-[25%] rounded-b-[10px] bg-primary flex items-center justify-between px-6">
                <div className="flex flex-col">
                  ...
                  <p
                    className="ml-[8.5rem] mt-1 mb-4 font-light text-[13px] group cursor-pointer"
                    onClick={() =>
                      navigate(`/${communityName}/joinedCommunity`)
                    }
                  >
                    <span className="font-semibold opacity-100 mr-1">
                ...
                <div className="flex gap-2 items-center text-[14px]">
                  <button
                    className="w-[3.6rem] h-[2.2rem] border rounded-full hover:text-emerald-green hover:border-emerald-green transition-all duration-100"
                    onClick={() => setPopup(true)}
                  >
                    Post
                  </button>
                  <button className="w-[3.6rem] h-[2.2rem] border rounded-full hover:text-emerald-green hover:border-emerald-green transition-all duration-100">
                    Join
                  </button>
                  {/* <button className="w-[4rem] h-[2.5rem] border rounded-full">...</button> */}
                  <EllipsisHorizontalCircleIcon
                    className="size-9 hover:text-emerald-green transition-all duration-100 cursor-pointer stroke-1"
      ...
      {popup && (
        <div
          className={`fixed inset-0 backdrop-brightness-50 backdrop-blur-[1px] z-50`}
        ></div>
      )}
      <CreatePostPopup setPopup={setPopup} popup={popup} />
      {openSetting && (
        <div
          className={`fixed inset-0 backdrop-brightness-50 backdrop-blur-[1px] z-50`}
        ></div>
