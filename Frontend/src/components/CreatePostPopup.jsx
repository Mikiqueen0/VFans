...
export default function CreatePostPopup({ setPopup, popup }) {
  ...
  const [filteredCommunityList, setFilteredCommunityList] =
    useState(communityList); // filtered community list
  ...
  useEffect(() => {
    if (communityID) {
      const communityFilter = communityList.filter(
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          if (img.width > 1500 || img.height > 1500) {
            ...
            // If image is within dimensions, set it
  ...
  const handleSubmitPost = async (e) => {
    ...
    try {
      ...
      const createPost = await axios.post("/post/create", {
        ...postData,
        image: uploadedFilesUrls,
        userID: user._id,
      });
  ...
  return (
    <form
      ...
      className={`fixed overflow-y-scroll inset-0 h-full flex flex-col justify-center items-center max-lg:px-[2rem] px-[10rem] xl:px-[4rem] z-50 overflow-x-hidden duration-[200ms] ${
        popup ? "scale-100 opacity-100" : "scale-0 opacity-0"
      }`}
    >
      {/* <div className={`fixed inset-0 backdrop-brightness-50 backdrop-blur-[1px]`}></div> */}
      <div
        className={`bg-primary rounded-[10px] flex flex-col max-md:w-full w-[700px] transform`}
      ...
      >
        <div className="grid grid-cols-[1/3_1/3_1/3] items-center px-6 py-4 border-b border-white border-opacity-10">
          <p className="col-start-2 text-white text-opacity-90 flex justify-center">
            Create Post
          </p>
          <div className="col-start-3 flex justify-end">
            {/* <img src="./assets/images/cross.png" alt="cross" className="w-5 h-5" /> */}
            <XMarkIcon
              className="size-8 text-white hover:cursor-pointer"
        ...
        <div className="mx-6 mt-2">
          ...
          <div className={`h-12 w-[50%] relative my-2`}>
            <div
              ...
              className={`appearance-none bg-transparent text-base text-white text-opacity-80 h-full w-full inline-flex items-center py-2 px-4 focus:outline-none border justify-between border-emerald-green border-opacity-70 ${
                communityDropdown
                  ? "rounded-b-[0px] rounded-t-[20px]"
                  : "rounded-[20px]"
              }`}
            >
              {communityDropdown ? (
                ...
                <p className="flex items-center">
                  {communitySelect.name ? (
                    <>
                      <img
                        src={communitySelect.image}
                        alt=""
                        className="bg-emerald-green rounded-full size-6 mr-2"
                      />
                      {communitySelect.name}
                    </>
                  ) : (
              ...
              <div className="bg-dark-background absolute w-full border border-t-0 border-emerald-green overflow-y-scroll max-h-[200px]">
                {filteredCommunityList.map((community, index) => {
                  return (
                    <div
                      key={index}
                      className="flex items-center px-4 py-3 gap-3 group cursor-pointer hover:bg-darkest-black"
                      onClick={() => handleCommunitySelect(community)}
                    >
                      <img
                        src={community.image}
                        alt=""
                        className="rounded-full size-8 bg-emerald-green object-cover"
                      />
                      <p className="text-white opacity-80 group-hover:text-emerald-green duration-100 flex-grow">
                        {community.name}
                      </p>
                    </div>
                  );
          ...
          <div className="mb-4 grid grid-cols-[5rem_5rem]">
            <div
              onClick={() => toggleTab(1)}
              className={`p-2 px-4 text-opacity-80 cursor-pointer text-white text-center transition-colors duration-100 hover:bg-dark-background active:bg-darkest-black ${
                toggleTabState === 1
                  ? "bg-transparent border-b-[3px] border-emerald-green"
          ...
          <div
            className={`transition-all duration-[400ms] ${
              toggleTabState === 1 ? "opacity-100" : "opacity-0"
            }`}
          >
            {toggleTabState === 1 && (
              <div>
                ...
                <div className="mt-2">
                  <section className="bg-primary rounded-[10px] shadow-md shadow-darkest-black">
                    <label
                      onClick={() => setTagCheckbox(!tagCheckbox)}
                      className="flex justify-between items-center h-10 cursor-pointer text-white text-opacity-80 font-normal text-lg p-3 rounded-[10px] bg-primary"
                    >
                      Tag
                    ...
                    <div
                      className={`block overflow-hidden py-0 transition-all duration-500 ${
                        tagCheckbox ? "max-h-80" : "max-h-0"
                      }`}
                    >
                      <div className="bg-dark-background min-h-[2.8rem] rounded-[10px] mt-1 flex flex-wrap gap-2 items-center p-2">
                        ...
                        {postData.tag.map((tag, index) => (
                          <div
                            key={index}
                            className="bg-emerald-green rounded-[4px] flex px-[0.3rem] py-1"
                          >
                        ...
                        {tagData.map((tag, index) => (
                          <button
                            ...
                            className={`text-sm font-medium text-${
                              postData.tag.includes(tag) ? "black" : "white"
                            } text-opacity-80 bg-${
                              postData.tag.includes(tag)
                                ? "emerald-green"
                                : "dark-background"
          ...
          <div
            className={`transition-all duration-[400ms] ${
              toggleTabState === 2 ? "opacity-100" : "opacity-0"
            }`}
          >
            {toggleTabState === 2 && (
                ...
                <div className="mt-2">
                  <div className="flex items-center justify-center w-full">
                    <label
                      ...
                      className="flex flex-col items-center justify-center w-full h-40 border-2 border-white border-dashed border-opacity-60 rounded-lg cursor-pointer bg-dark-background dark:hover:border-opacity-40 hover:bg-lighter-primary"
                    ...
                    >
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <PhotoIcon
                          className="size-14 p-2 text-white text-opacity-60"
                          aria-hidden="true"
                        />
                        <p className="mb-2 text-sm text-white text-opacity-60">
        ...
        <button
          type="submit"
          className="bg-emerald-green rounded-[20px] text-[16px] font-medium text-black text-opacity-[78%] text-lg py-4 px-6 mt-4 mx-6 mb-6 flex justify-center self-end max-w-[10rem] w-full"
        >
          Post
        </button>
      </div>
